...
public static class NativeFilePicker
{
	...
	[System.Runtime.InteropServices.DllImport( "__Internal" )]
    private static extern void _NativeFilePicker_OpenFolder(string subFolder);
#endif
    ...
    [CanBeNull]
    public static string ConvertExtensionToFileType([CanBeNull] string extension)
    {
    ...
    public static Permission PickFile([CanBeNull] FilePickedCallback callback, [NotNull] string[] allowedFileTypes)
    {
    ...
    public static Permission PickMultipleFiles([CanBeNull] MultipleFilesPickedCallback callback,
        [NotNull] string[] allowedFileTypes)
    {
        if (allowedFileTypes == null || allowedFileTypes.Length == 0)
            throw new ArgumentException("Parameter 'allowedFileTypes' is null or empty!");
    ...
    public static Permission ExportFile([NotNull] string filePath, [CanBeNull] FilesExportedCallback callback = null)
    {
        ...
        if (result == Permission.Granted && !IsFilePickerBusy())
        {
            if (CanExportFiles())
            {
                ...
                string destination = UnityEditor.EditorUtility.SaveFilePanel("Select destination",
                    Path.GetDirectoryName(filePath), Path.GetFileName(filePath), extension);
    ...
    public static Permission ExportMultipleFiles([NotNull] string[] filePaths,
        [CanBeNull] FilesExportedCallback callback = null)
    {
        ...
        if (result == Permission.Granted && !IsFilePickerBusy())
        {
            ...
            {
#if UNITY_EDITOR
                string destination = UnityEditor.EditorUtility.OpenFolderPanel("Select destination",
    ...
    public static void OpenAppFolder(string subFolder = "")
    {
#if !UNITY_EDITOR && UNITY_IOS
        _NativeFilePicker_OpenFolder(subFolder);
#endif
    }
