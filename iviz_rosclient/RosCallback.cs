...
{
    public abstract void Handle(in T message, IRosConnection info);
}
...
public sealed class ActionRosCallback<T> : RosCallback<T>
{
    ...
    public override void Handle(in T message, IRosConnection info) => action(in message, info);
}
...
public sealed class DirectRosCallback<T> : RosCallback<T>
{
    ...
    public override void Handle(in T message, IRosConnection _) => action(message);
}